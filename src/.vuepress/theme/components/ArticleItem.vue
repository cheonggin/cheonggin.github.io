<template>
  <article class="article">
    <div v-if="info.sticky" class="sticky">
      <Xicons icon="ArrowBarToUp" class="sticky-icon" iconSize="24"></Xicons>
    </div>

    <header class="title">
      <RouterLink :to="path">
        <Xicons v-if="info.isEncrypted" icon="ArrowBarToUp"></Xicons>
        <SlideIcon v-if="info.type === 'slide'" />
        {{ info.title }}
      </RouterLink>
    </header>
    <div class="excerpt" v-if="info.excerpt">{{ info.excerpt }}</div>
    <PageInfo :config="config" :items="items" />
  </article>
</template>

<script setup lang="ts">
import { toRef, unref } from 'vue'
import { useArticleInfo } from '@theme-hope/module/blog/composables'
import { SlideIcon } from '@theme-hope/module/blog/components/icons'
import PageInfo from '@theme-hope/module/info/components/PageInfo'

import type { PropType } from 'vue'
import type { ArticleInfo } from 'vuepress-theme-hope'

const props = defineProps({
  info: {
    type: Object as PropType<ArticleInfo>,
    required: true
  },
  path: { type: String, required: true }
})
const info = toRef(props, 'info')
let { config, items } = useArticleInfo(info)
config.value = unref(config)
</script>

<style lang="scss">
.article {
  position: relative;

  box-sizing: border-box;

  width: 100%;
  margin-bottom: 2rem;
  padding: 1rem 1.25rem;
  border-radius: 0.4rem;

  background: var(--bg-color-float);
  box-shadow: 0 1px 3px 1px var(--card-shadow);

  text-align: left;

  @media (max-width: hope-config.$mobile) {
    border-radius: 0;
  }

  &:last-child {
    margin-bottom: 0;
  }

  &:hover {
    box-shadow: 0 2px 6px 2px var(--card-shadow);
  }

  .sticky {
    position: absolute;
    top: 0;
    right: 0;

    width: 1.5rem;
    height: 1.5rem;

    color: var(--theme-color);
    width: 0;
    height: 0;
    border-color: transparent var(--theme-color) transparent transparent;
    border-style: solid;
    border-width: 0 40px 40px 0;

    &-icon {
      position: absolute;
      top: 0;
      right: -45px;
      color: #fff;
    }
  }

  .title {
    position: relative;

    display: inline-block;

    font-size: 1.28rem;
    line-height: 2rem;
    margin-bottom: 0.5rem;

    &::after {
      content: '';

      position: absolute;
      bottom: 0;
      left: 0;

      width: 100%;
      height: 2px;

      background: var(--theme-color);

      visibility: hidden;

      transition: transform 0.3s ease-in-out;
      transform: scaleX(0);
    }

    &:hover {
      cursor: pointer;

      &::after {
        visibility: visible;
        transform: scaleX(1);
      }
    }

    a {
      color: inherit;
    }

    .lock-icon,
    .slides-icon {
      position: relative;
      bottom: -0.125em;

      display: inline-block;
      vertical-align: baseline;

      width: 1.25em;
      height: 1.25em;
      margin-right: 0.25em;

      color: var(--theme-color);
    }
  }

  .excerpt {
    overflow: hidden;
    line-height: 1.7;

    h1 {
      display: none;
    }

    // stylelint-disable selector-max-compound-selectors
    h1 + p {
      margin-top: 0.5em;
    }
    // stylelint-enable selector-max-compound-selectors

    p:first-child {
      margin-top: 0.5em;
    }

    p:last-child {
      margin-bottom: 0.5em;
    }

    // code block fix
    pre {
      margin: 0.85rem 0;
      padding: 1.25rem 1.5rem;
      line-height: 1.4;
    }

    // line number fix
    .line-numbers-mode {
      // stylelint-disable selector-max-compound-selectors
      pre {
        padding-left: calc(var(--line-numbers-width) + 1rem);
      }
      // stylelint-enable selector-max-compound-selectors
    }

    // hide code demo
    .code-demo-wrapper {
      display: none;
    }

    // footnote fix
    .footnote-anchor {
      display: none;
    }

    section.footnotes {
      display: none;
    }
  }
}
</style>
