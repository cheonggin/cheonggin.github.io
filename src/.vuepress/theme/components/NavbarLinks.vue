...
import { useNavLink } from 'vuepress-theme-reco/lib/client/composables'
import { useThemeLocaleData } from '@vuepress/plugin-theme-data/lib/client'
import { usePageData } from '@vuepress-reco/vuepress-plugin-page/lib/client/composable'
import { convertToPinyin } from '@vuepress-reco/shared'
import { resolveRepoType } from 'vuepress-theme-reco/lib/client/utils'
import DropdownLink from 'vuepress-theme-reco/lib/client/components/DropdownLink.vue'
...
const useNavbarConfig = (): ComputedRef<ResolvedNavbarItem[]> => {
  ...
  const parseCategories = computed(() => {
    const categoriesFirstItem = Object.values(
      classificationSummary.value?.categories?.items
    )[0].label
    const tagsFirstItem = Object.values(
      classificationSummary.value?.tags?.items
    )[0].label
    ...
    return [
      {
        text: '分类',
        icon: 'LayoutGrid',
        link: `/categories/${convertToPinyin(categoriesFirstItem)}/1`
      },
      {
        text: '标签',
        icon: 'Tag',
        link: `/tags/${convertToPinyin(tagsFirstItem)}/1`
      }
    ]
  })
